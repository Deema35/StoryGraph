...
void AHUD_StoryGraph::OpenDialogOrOpenPlaceTriggerMessages(TArray<UStoryGraphObjectWithSceneObject*>& OwningObjects)
{
	...
	if (CurrentStoryGraphObjects.Num() > 0)
	{
		if (Dialogs.Num() > 0)
		{
			if (UStoryGraphCharacter* Character = Cast<UStoryGraphCharacter>(CurrentStoryGraphObjects[0]))
			{
		...
		else
		{
			...
			{
				if (UStoryGraphCharacter* Character = Cast<UStoryGraphCharacter>(CurrentStoryGraphObjects[i]))
				{
					...
					else
					{
						UE_LOG(LogCategoryStoryGraphPluginRuntime, Warning,
						       TEXT("Character %s dont have default answer"), *Character->ObjName.ToString());
					}
				}
				else if (UStoryGraphPlaceTrigger* PlaceTrigger = Cast<UStoryGraphPlaceTrigger>(
					CurrentStoryGraphObjects[i]))
				{
					...
					{
						UE_LOG(LogCategoryStoryGraphPluginRuntime, Warning,
						       TEXT("PlaceTrigger %s dont have default answer"), *PlaceTrigger->ObjName.ToString());
					}
				}
			}
		}
...
void AHUD_StoryGraph::GetRootDialogs(TArray<UDialogObject*>& Dialogs)
{
	for (int i = 0; i < CurrentStoryGraphObjects.Num(); i++)
	{
		UStoryGraphCharacter* Character = Cast<UStoryGraphCharacter>(CurrentStoryGraphObjects[i]);
		UStoryGraphPlaceTrigger* PlaceTrigger = Cast<UStoryGraphPlaceTrigger>(CurrentStoryGraphObjects[i]);
		if (Character)
		{
			for (int j = 0; j < Character->GraphNodes.Num(); j++)
			{
				if (UDialogStartNode* DialogStartNode = Cast<UDialogStartNode>(Character->GraphNodes[j]))
				{
		...
		{
			for (int j = 0; j < PlaceTrigger->GraphNodes.Num(); j++)
			{
				if (UDialogStartNode* DialogStartNode = Cast<UDialogStartNode>(PlaceTrigger->GraphNodes[j]))
				{
					if (DialogStartNode->IsActive)
					{
						Dialogs.Add(DialogStartNode->Dialogs[0]);
	...
	{
		return ((UDialogStartNode*)Dialog1.DialogNode)->DialogPriority < ((UDialogStartNode*)Dialog2.DialogNode)->
			DialogPriority;
	};
...
void AHUD_StoryGraph::GetNextDialogFromBrunch(UCustomNodeBase* Brunch, TArray<UDialogObject*>& Dialogs)
{
	...
	while (true)
	{
		...
		case EPerformNodeResult::Successed:
...
void AHUD_StoryGraph::PreInitializeComponents()
{
	...
	if (GetGameInstance())
	{
		...
		else UE_LOG(LogCategoryStoryGraphPluginRuntime, Error,
		            TEXT("You must set GameIstans clas as USaveGameInstance"));
	}
}
