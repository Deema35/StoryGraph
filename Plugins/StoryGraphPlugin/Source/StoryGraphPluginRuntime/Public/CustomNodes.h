...
2) adjust UCustomNodeBase::GetInsertNodeType.
...
6) If node is depended type, you must add member in UStoryGraphObject::DependedNodes Array (from file StoryGraphObject.h).
*/
...
enum class ENodeType : uint8
{
	...
	AddMessageBranch,
	AddDialogFromDialog,
	SendMessageToLevelBlueprint
};

UENUM()
enum class EInsertNodeType
{
	NotDefine,
...
struct FStoryGraphPin
{
	...
	FStoryGraphPin(EEdGraphPinDirection Direction_, EPinDataTypes PinDataType_) : Direction((int)Direction_),
	                                                                              PinDataType((int)PinDataType_)
	{
	}
...
public:
	...
	FPropertyUpdateDelegate PropertyUpdateDelegate; //Update detail panel
...
public:
	...
	UCustomNodeBase() : pGraphObject(nullptr), pStoryGraph(nullptr)
#if WITH_EDITORONLY_DATA
	...
	static FLinearColor GetColorFromNodeType(ENodeType NodeType, int ColorNumber = 0);

	static EInsertNodeType GetInsertNodeType(ENodeType NodeType);
	//If node Dependent you must add this node to UStoryGraphObject::DependedNodes appropriate object
	...
	virtual void RefreshColor() { NodeColor = UCustomNodeBase::GetColorFromNodeType(NodeType); }
	...
	void DeleteLinkToNode(UCustomNodeBase* NodeLink);
#endif //WITH_EDITORONLY_DATA
};
...
class STORYGRAPHPLUGINRUNTIME_API UStoryVerticalNodeBase : public UCustomNodeBase
	//Base class for nods witch have vertical pins
{
	GENERATED_BODY()
public:

	UStoryVerticalNodeBase() : bPerformNode(false), pQuestOwner(nullptr), pQuestPhase(nullptr)
	{
	}
...
UCLASS()
class STORYGRAPHPLUGINRUNTIME_API UStoryHorizontalNodeBase : public UCustomNodeBase
	//Base class for nods witch have horizontal pins
{
	GENERATED_BODY()
public:
...
public:
	...
	virtual EPerformNodeResult PerformNode() override { return EPerformNodeResult::Successed; }
#if WITH_EDITORONLY_DATA
	virtual void InitNode(class UStoryGraphObject* pGraphObject_) override;
...
UCLASS()
class STORYGRAPHPLUGINRUNTIME_API USetSceneObjectActiveNode : public UStoryHorizontalNodeBase
{
	GENERATED_BODY()
public:
...
public:
	...
	virtual FText GetNodeTitle() const override;
#if WITH_EDITORONLY_DATA
	void SetCurrentDialog(UDialogStartNode* SelectedDialog_);
...
public:
	UPROPERTY()
	int NewCurrentInventoryItemState;
...
UCLASS()
class STORYGRAPHPLUGINRUNTIME_API USendMessageToLevelBlueprintNode : public UStoryHorizontalNodeBase
{
	GENERATED_BODY()
public:
	UPROPERTY(EditAnywhere, BlueprintReadOnly)
